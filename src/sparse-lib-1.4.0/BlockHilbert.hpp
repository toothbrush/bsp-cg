...
template< typename T >
class BlockHilbert: public SparseMatrix< T, unsigned long int > {
   ...
   protected:
	...
	static const ULI MAX_DEPTH = 64; // NOTE: This CANNOT be enlarged, unless unsigned long ints are larger than 64 bits!
...
//			std::cout << "\t  --------------------" << std::endl;
   ...
   public:
	 ...
	 */
	BlockHilbert( std::vector< Triplet< T > >& input, ULI m, ULI n, T zero ) {
		bisection = 0;
		load( input, m, n, zero );
	}
	 ...
	 *  @param zero What elements is considered to-be zero.
	 */
	BlockHilbert( std::vector< Triplet< T > >& input, ULI m, ULI n, char bisect, T zero ) {
		bisection = bisect;
		load( input, m, n, zero );
	}
